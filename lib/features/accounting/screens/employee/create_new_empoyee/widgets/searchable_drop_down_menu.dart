...
class _SearchableDropdownFieldState<T> extends State<SearchableDropdownField<T>> {
  ...
  void initState() {
    ...
    _controller = TextEditingController(
      text: widget.selectedItem == null ? '' : widget.labelBuilder(widget.selectedItem!),
    );
  ...
  void didUpdateWidget(covariant SearchableDropdownField<T> oldWidget) {
    ...
    if (widget.selectedItem != oldWidget.selectedItem) {
      _controller.text = widget.selectedItem == null ? '' : widget.labelBuilder(widget.selectedItem!);
    }
  ...
  void _filter(String query) {
    setState(() {
      _filteredItems = widget.items
          .where((item) => widget.labelBuilder(item).toLowerCase().contains(query.toLowerCase()))
          .toList();
    });
                ...
                itemBuilder: (context, index) {
                  ...
                  return InkWell(
                    ...
                    child: Padding(
                      ...
                      child: Text(widget.labelBuilder(item), style: const TextStyle(fontSize: 14)),
                    ),
                  );
                },
              ),
      ...
      child: MouseRegion(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            suffixIcon: _isHovering ? Icon(Icons.arrow_drop_down) : null ,
            isDense: true,
            isCollapsed: true,
            contentPadding: EdgeInsets.zero,
            hintText: widget.hintText,hintStyle: TextStyle(color: APPColors.hintText),
